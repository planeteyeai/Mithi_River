...
const Dashboard = () => {
  ...
  useEffect(() => {
    ...
    let cancelled = false
    const loadMithiRiver = async () => {
      try {
        const response = await fetch('/asset/mula-mutha-river.kml')
        if (!response.ok) throw new Error('Failed to load Mithi River')
        const content = await response.text()
        if (cancelled) return
        setUploadedKML({
          name: 'Mithi River.kml',
          displayName: 'Mithi River',
          content,
        })
        setDrawnGeometry(null)
        setIsDrawing(false)
        setKmlDownloadName('mithi-river')
      } catch (error) {
        console.error(error)
      }
    }
...
<kml xmlns="http://www.opengis.net/kml/2.2">
  ...
  const isMulaMuthaRiver =
    uploadedKML?.displayName === 'Mithi River' ||
    uploadedKML?.displayName === 'Mula-Mutha River-DISABLED' ||
    uploadedKML?.name?.toLowerCase().includes('mithi') ||
