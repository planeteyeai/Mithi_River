...
const NDSI_SALINITY_IMG_SOURCE = 'src-ndsi-salinity-img'
const NDSI_SALINITY_RASTER = 'lyr-ndsi-salinity-raster'
const NDSI_SALINITY_FILL = 'lyr-ndsi-salinity-fill'
const NDSI_SALINITY_LINE = 'lyr-ndsi-salinity-line'
const NDSI_SALINITY_JSON_URL = '/asset/mula-mutha-ndsi-salinity.json'
...
// Default box is the Mithi River lithology AOI (Mumbai).
const MULA_MUTHA_IMAGE_COORDS = [
  [72.82722078868055, 19.094260871183593],
  [72.88661777120646, 19.094260871183593],
  [72.88661777120646, 19.038481978963834],
  [72.82722078868055, 19.038481978963834],
]
...
const ensureOverlayLayers = (map) => {
  ...
  // Mithi ships salinity as a GroundOverlay raster; keep empty polygon layers
  // for older polygon-mode assets that still have features.
  ...
  // Volume surface is optional (Mithi pack is class-only); placeholder uses class PNG.
  ensureImageRaster(SILT_VOLUME_SOURCE, SILT_VOLUME_RASTER, '/asset/mula-mutha-silt-class-2026-06.png', {
    'raster-opacity': 0.82,
    'raster-resampling': 'linear',
  })
  ensureImageRaster(NDSI_SALINITY_IMG_SOURCE, NDSI_SALINITY_RASTER, '/asset/mithi-salinity-overlay.png', {
    'raster-opacity': 0.88,
    'raster-resampling': 'nearest',
  })
...
const MapComponent = ({
  ...
  useEffect(() => {
    ...
    const setPolyVisibility = (visible) => {
      const value = visible ? 'visible' : 'none'
      ;[NDSI_SALINITY_FILL, NDSI_SALINITY_LINE].forEach((id) => {
        if (map.getLayer(id)) map.setLayoutProperty(id, 'visibility', value)
      })
    }
    const setRasterVisibility = (visible) => {
      if (map.getLayer(NDSI_SALINITY_RASTER)) {
        map.setLayoutProperty(NDSI_SALINITY_RASTER, 'visibility', visible ? 'visible' : 'none')
      }
    }

    if (!showNdsiSalinityLayer) {
      setPolyVisibility(false)
      setRasterVisibility(false)
    ...
    const load = async () => {
      try {
        const meta = await fetchAssetJson(NDSI_SALINITY_JSON_URL, 'NDSI salinity')
        if (cancelled) return

        if (meta.mode === 'raster' && meta.raster) {
          const source = map.getSource(NDSI_SALINITY_IMG_SOURCE)
          const coords = meta.imageCoordinates
          if (source?.updateImage && isValidImageCoordinates(coords)) {
            source.updateImage({ url: meta.raster, coordinates: coords })
          }
          setPolyVisibility(false)
          setRasterVisibility(true)
        } else {
    ...
    const onClick = (event) => {
      ...
      ndsiSalinityPopupRef.current = new Popup({ closeButton: true, maxWidth: '240px', offset: 10 })
        .setLngLat(event.lngLat)
        .setHTML(
          `<div style="font:600 13px Inter,system-ui,sans-serif;color:#0d2436">Salinity ${props.label || 'class'}</div>` +
            `<div style="margin-top:4px;font:500 11px Inter,system-ui,sans-serif;color:#3a5c73">Range: ${props.range || '—'}</div>` +
            `<div style="margin-top:2px;font:500 11px Inter,system-ui,sans-serif;color:#6b8798">Relative classes · Estimated</div>`,
        )
        .addTo(map)
    }
    const onEnter = () => {
  ...
  useEffect(() => {
    ...
    const publish = (note) => {
      const key = note
        ? `${note.layerId}:${note.label}:${Math.round((note.x || 0) / 8) * 8}:${Math.round((note.y || 0) / 8) * 8}`
        : ''
      if (key === classNoteKeyRef.current) return
      classNoteKeyRef.current = key
      setClassNote(note)
      // Layers panel highlight only needs class identity, not pixel position.
      publishClassHover(
        note
          ? { layerId: note.layerId, label: note.label, color: note.color, value: note.value }
          : null,
      )
    }
  ...
  return (
    <div className="map-container">
      ...
      {classNote && (
        <div
          className="map-class-note is-follow"
          aria-live="polite"
          style={{
            left: Math.max(12, (classNote.x || 0) + 14),
            top: Math.max(12, (classNote.y || 0) + 14),
          }}
        >
